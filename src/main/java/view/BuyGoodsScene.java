...
public class BuyGoodsScene {
    ...
    public BuyGoodsScene(Stage window, GameEngine gameEngine) throws Exception {
        this.window = window;
        this.backgroundFactory = new LightBackgroundFactory();
        this.buttonFactory = new BrownButtonFactory();
        this.window.setTitle("Buy Goods");
        ...
        button11.setOnAction(event -> {
            try {
                window.setScene(new FindMarketLocationScene(window, gameEngine).getScene());
                window.setTitle("View Market Place Page");
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
        ...
        button12.setText("Buy Fuel");
        button12.setOnAction(event -> {
            try {
                window.setScene(new BuyFuelScene(window, gameEngine).getScene());
                window.setTitle("Buy Fuel Page");
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
        ...
        button13.setOnAction(event -> {
            try {
                window.setScene(new BuyOtherScene(window, gameEngine).getScene());
                window.setTitle("Buy Other Goods");
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
