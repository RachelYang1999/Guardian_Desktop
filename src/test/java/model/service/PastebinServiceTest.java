...
public class PastebinServiceTest {
  ...
  public void init() {
    ...
    when(onlinePastebinAPIStrategy.getPastebinLink("correct token", "correct content"))
        .thenReturn(dummyResponseSuccessful);
    when(offlinePastebinAPIStrategy.getPastebinLink("correct token", "correct content"))
        .thenReturn(dummyResponseSuccessful);

    when(onlinePastebinAPIStrategy.getPastebinLink("incorrect token", "correct content"))
        .thenReturn(dummyResponseInvalidToken);
    when(offlinePastebinAPIStrategy.getPastebinLink("incorrect token", "correct content"))
        .thenReturn(dummyResponseInvalidToken);

    when(onlinePastebinAPIStrategy.getPastebinLink("irregular token", "irregular content"))
        .thenReturn(dummyResponseIrregular);
    when(offlinePastebinAPIStrategy.getPastebinLink("irregular token", "irregular content"))
        .thenReturn(dummyResponseIrregular);
  }
