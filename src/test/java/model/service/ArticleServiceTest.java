...
public class ArticleServiceTest {
  ...
  public void init() {
    ...
    when(onlineGuardianAPIStrategy.searchByTag("correct token", "sausage", 1))
        .thenReturn(successfulResponse);
    when(offlineGuardianAPIStrategy.searchByTag("correct token", "sausage", 1))
        .thenReturn(successfulResponse);

    when(onlineGuardianAPIStrategy.searchByTag("incorrect token", "sausage", 1))
        .thenReturn(invalidTokenResponse);
    when(offlineGuardianAPIStrategy.searchByTag("incorrect token", "sausage", 1))
        .thenReturn(invalidTokenResponse);

    when(onlineGuardianAPIStrategy.searchByTag("correct token", "no result tag", 1))
        .thenReturn(noResultResponse);
    when(offlineGuardianAPIStrategy.searchByTag("correct token", "no result tag", 1))
        .thenReturn(noResultResponse);

    when(onlineGuardianAPIStrategy.searchByTag("correct token", "irregular tag", 1))
        .thenReturn(irregularResponse);
    when(offlineGuardianAPIStrategy.searchByTag("correct token", "irregular tag", 1))
        .thenReturn(irregularResponse);
